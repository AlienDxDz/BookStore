...
if (isset($_SESSION["admin"])) {
	...
	echo "<div></div>";
	echo "<div class='hr'><hr></div>";
	echo "<div><b>Неподтвержденные комментарии</b></div>";
	echo "<div class='hr'><hr></div>";
	echo "<div></div>";
	echo "</div>";

	$sasas = $pdo->query("SELECT * FROM commentcheck;");
  	$rasas = $sasas->fetchAll();

  	if ($rasas == null) {
  		echo "<p class='bad'>В базе нет ни одной неподтвержденный комментарии. Пользователи еще не оставили отзывы о книге. </p>";
  	}
  	...
  	foreach ($rasas as $ke) {
		...
		echo "<input type='hidden' name='idComment' value='".$ke['id']."'>";
		echo "<input class='btnff' type=\"submit\" value='Подтверждать'>";
		echo "</form>";
		echo "</div>";

		echo "<div>";
        echo "<form method='POST' action='deletebinAdmincheck.php'>";
		echo "<input type='hidden' name='idComment' value='".$ke['id']."'>";
		echo "<input class='btnf' type=\"submit\" value='Удалить'>";
		echo "</form>";
		echo "</div>";
